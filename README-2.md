...
The site now opens with a short (~3s) cinematic rocket launch before
revealing the dashboard:

- **Systems check → Ignition → Liftoff → Welcome** — a line-art rocket
  on a glowing pad shakes to life, engines flare, then it accelerates
  smoothly off-screen against the site's real starfield/constellation
  backdrop (no separate assets — same canvases used throughout the site).
- **Skip intro** button is available immediately (also works via `Esc`),
  for visitors who prefer to jump straight to the dashboard.
- Fully disabled under `prefers-reduced-motion` — those visitors go
  straight to the site with no delay.
- Pure CSS transform/opacity animation (no extra JS render loop), scaled
  down for small screens, and the loader is removed from rendering
  entirely once it's finished, so it never costs anything after the
  intro plays.
