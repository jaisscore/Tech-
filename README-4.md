...
**Type** — Sora for display/headings, Plus Jakarta Sans for body copy,
JetBrains Mono for data readouts, labels, and status text.

**Logo** — a minimal ringed mark (Saturn / orbital-ring / Dyson Sphere
inspired): a thin circle (the sphere), a wider ellipse (the orbital ring),
and a single orbiting point (a collector satellite), all in metallic gold.
The same mark is used everywhere — favicon, nav, and the full lockup — at
different scales rather than as separate designs, which is what keeps it
scalable and recognizable. Delivered as `logo-mark.svg` (icon only) and
